    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="content-page-header">
                ...
                <div class="list-btn" v-if="can('create', 'product')">
        ...
        <div class="modal modal-top fade" id="transfertModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <form class="modal-content" @submit.prevent="TransfertFunction">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5">Transférer vers le magasin</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" v-model="transfertData.motif" rows="2" placeholder="Ex: Réapprovisionnement magasin"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-secondary me-3" data-bs-dismiss="modal">Fermé</button>
...
<script setup>
    ...
    const columns = ref([
        ...
        { title: 'Action', data: null, render: (data,type,row) => {
            ...
            const peutVoir = can('read', 'product')
            const peutModifier = can('update', 'product')
            const peutSupprimer = can('delete', 'product')
    ...
    async function AddProductFunction() {
        ...
        if (allEmpty) {
            isLoader.value = true;
            await postData('/products/boutiqueStore',data.value).then(res=>{
                if (res.status === 200) {
    ...
    async function UpdateProductFunction() {
        isLoader.value = true;
        await putData('/products/boutiquePut/'+data.value.id,data.value).then(res=>{
            if (res.status === 200) {
                isLoader.value = false;
                data.value = {
                    code_barre:'',
    ...
    async function TransfertFunction() {
        ...
        await postData('/transferts/vers-magasin', transfertData.value).then(res => {
            if (res.status === 200) {
